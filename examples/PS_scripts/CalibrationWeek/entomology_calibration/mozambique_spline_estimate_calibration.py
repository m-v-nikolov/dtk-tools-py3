...
def get_spline_values(ref, spec):
    ref = ref.reset_index()
    ...
    # temp_list += [temp_list.pop(0)]
    ref['Counts_normalized'] = temp_list
    ref['Minimum'] = 0*len(ref)
    ref['Maximum'] = [50*i for i in temp_list]
    ref['Guess'] = [0.5*i for i in temp_list]

    ref.index = [i for i in range(12)]
    return ref


ref = sites[0].analyzers[0].reference.copy()

params1 = [None]*len(specs)*12
for s, species in enumerate(specs):
    spline = get_spline_values(ref, species)
    for i in range(1, 13):
        params1[12*s + i - 1] = {
            'Name': '%s%i' % (species, i),
            'Dynamic': True,
            'Guess': spline['Guess'][i-1],
            'Min': spline['Minimum'][i-1],
            'Max': spline['Maximum'][i-1]
        }
...
hab_val = {'gambiae': 2.2e7, 'funestus': 2e3}
...
def map_sample_to_model_input(cb, sample):
    ...
    tags.update(cb.set_param("Simulation_Duration", 3*365))
...
calib_manager = CalibManager(name='MozambiqueEntoCalibTest1_%s' %specs[0],    # <-- Please customize this name
                             config_builder=cb,
