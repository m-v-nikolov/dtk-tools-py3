...
class PrognosisAnalyzer(BaseAnalyzer):
    ...
    def __init__(self,
        ...
        super(PrognosisAnalyzer, self).__init__()
    ...
    def apply(self, parser):
        ...
        age = hiv_infections['Age']
        prognosis = hiv_infections['Prognosis']
    ...
    def combine(self, parsers):
        ...
        for (key, prognosis) in self.data.items():
            ...
            try:
                self.results[abi] = stats.kstest(prognosis, self.fun[abi], self.alpha)
    ...
    def plot_figure(self):
        ...
        for idx, key in enumerate(sorted(keys, key=lambda x: x[1])):
            ...
            plt.title(
                "Age bin %s: %s (p=%0.2f)" % (abi, valid_str[self.results[abi]['Valid']], self.results[abi]['P_Value']))
