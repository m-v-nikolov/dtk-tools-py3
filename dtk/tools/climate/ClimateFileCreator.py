...
class ClimateFileCreator:
    def __init__(self, nodes, prefix, suffix, original_data_years):
        """
        :param nodes: format -
            node1 = WeatherNode()
            node1.id = 340461479
    ...
    def generate_climate_files(self, output_path):
        ...
        for node in self.nodes:
            ...
            if rainfall_count > 0:
                ...
                count = air_temperature_count
            elif land_temperature_count > 0:
                count = land_temperature_count
        ...
        if len(rainfall_data) > 0:
            self.write_files(output_path, count, offset_string, rainfall_data, "rainfall")

        if len(air_temperature_data) > 0:
            self.write_files(output_path, count, offset_string, air_temperature_data, "air_temperature")

        if len(land_temperature_data) > 0:
            self.write_files(output_path, count, offset_string, land_temperature_data, "land_temperature")

        if len(humidity_data) > 0:
            self.write_files(output_path, count, offset_string, humidity_data, "humidity")
