...
class FidelityReportExperimentDefinition(defaultdict):
    ...
    """Constructs experiement definition dictionary based on the fidelity report .json config file or based on the 
    exp_def.json file generated by a previouse report run. """
    ...
    def init_experiment_definition(self, xd_json, mode, sweep, report_channel_list):
        ...
        # error messages correspond to 'dtk catalyst' word definitions
        modes = xd_json['modes']
        if run_mode_name not in modes:
            raise self.InvalidSelection('Invalid run mode: %s . Must be one of: %s' %
                                        (run_mode_name, list(modes.keys())))
        self.set_keys(modes[run_mode_name])

        sweeps = xd_json['sweeps']
        if sweep_type_name not in sweeps:
            raise self.InvalidSelection('Invalid sweep method: %s . Must be one of: %s' %
        ...
        if 'spatial' in self and self['spatial'] is True:
            ...
            if spatial_channel_names is None:
                spatial_channel_names = list(
                    channel_map[channel_map['Inset_Channel'].isin(self['inset_channel_names'])]['Spatial_Channel'])
