...
def add_immune_overlays_site(cb, tags, site):
    ...
    split_demog = demog_filename.split('.')
    prefix,suffix = split_demog[0],split_demog[1:]
    prefix = site + '_' + "immune_init_"
    demogfiles = [demog_filename]
    for tag in tags:
        #if 'demographics' not in prefix:
        #    raise Exception('add_immune_init function expecting a base demographics layer with demographics in the name.')
        #immune_init_file_name = prefix.replace("demographics","immune_init_" + tag, 1) + "."+suffix[0]
        immune_init_file_name = prefix + tag + '.'+suffix[0]
        demogfiles.append(immune_init_file_name)
...
def add_immune_overlays_sites(cb, tags):
    ...
    demogfiles = [demog_filename]
    for tag in tags:
        #Sinamalima_1_node_immune_init_x_0.2_const_4.0.json
        immune_init_file_name = tag[2] + '_immune_init_x_' + str(tag[0]) + '_const_' + str(tag[1]) + '.json'
...
def add_immune_init_all_const(cb, site, x_temp_habitats, const_habs):
    tags = []
    for x_temp_habitat in x_temp_habitats:
        for const_h in const_habs:
            tags.append( "x_" + str(x_temp_habitat) +  '_const_' + str(const_h))
    add_immune_overlays_site(cb, tags, site)

def add_immune_init_all_const_site(cb, tags):
    add_immune_overlays_sites(cb, tags)
