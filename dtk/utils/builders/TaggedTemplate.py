...
class TaggedTemplate(ITemplate.BaseTemplate):
    ...
    # May need to manipulate key here
        return param in self.tag_dict
    ...
    def get_param(self, param):
        ...
        """
        values = {}
        if param in self.tag_dict:
            for expanded_param in self.__expand_tags(param):
                value = self.__get_expanded_param(expanded_param)
                values[param] = value

            return values

        return super(TaggedTemplate, self).get_param(param)

    def __get_expanded_param(self, param):
        '''
        '''
        param_name = '.'.join( str(p) for p in param)
        logger.info( "[%s] Getting parameter %s." % (self.filename, param_name) )

        current_parameter = self.contents

        for path_step in param:
