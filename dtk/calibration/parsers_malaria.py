...
def get_paths_local(settings, iteration) :
    ...
    # WHY ARE WE EVEN DOING THIS RATHER THAN INDEXING ON SIM_ID?
    for sindex in range(len(subdirs)) :
        ...
        for p in range(numsamples) :
            for run_num in range(settings['sim_runs_per_param_set']) :
                ...
                for pname in paramnames :
                    print(pname)
                    print(config_params[pname])
                    print(samples[pname].values[p])
                    try:
                        param = float(config_params[pname])
                    except:
                        param = config_params[pname]
                    if abs(param - samples[pname].values[p]) > settings['ERROR'] :
                        match = False
                        break
