...
def add_drug_campaign(cb, drug_code, start_days, coverage=1.0, repetitions=3, interval=60, diagnostic_threshold=40, 
    ...
    elif campaign_type in ['fMDA', 'RCD'] :

        drug_configs = drug_configs_from_code(cb,'MDA_' + drugs)
        fmda_setup = fmda_cfg(drug_code.split('_')[1])
        ...
        if campaign_type == 'fMDA' :
            for start_day in start_days :
                add_diagnostic_survey(cb, coverage=coverage, repetitions=repetitions, tsteps_btwn=interval, target=target_group, start_day=start_day, 
                                      ...
                                      nodes=nodes, positive_diagnosis_config=fmda_setup )

        elif campaign_type in ['RCD'] :
            rcd_event = {   "Event_Name": "Trigger RCD", 
                            "class": "CampaignEvent",
            ...
            for snowball in range(snowballs) :
                ...
                add_triggered_survey(cb, coverage=coverage, start_day=start_days[0], 
                                     ...
                                     positive_diagnosis_config={ "Intervention_List" : [snowball_setup[snowball+1], fmda_setup] ,
